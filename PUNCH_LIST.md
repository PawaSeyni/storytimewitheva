...
- [x] **Guide character / mascot** — **Pixel the Butterfly** (`src/components/Pixel.tsx`). **Final illustrated art delivered and integrated** (`src/assets/pixel/*.svg`): 6 moods (hello / reading / praise / sleepy / pointing / listening) + a 24px-optimized inline reading variant, each rendered as an `<img>` (no SVG id collisions across instances). Placed at the Home hero + Profile empty states behind a `?pixel=1` preview flag (off for normal visitors). Brief: `docs/pixel-mascot-brief.md`. **Only remaining step: flip the default in `readEnabled()` to launch her for everyone** (owner go decision). Bonus poses (pointing → tap-to-translate, listening → read-aloud) available for future wiring.
