...
- [~] Profile auth — decision memo written (`docs/profile-auth-decision.md`). **Recommendation: keep localStorage-only** (profile stores zero PII — just book ids + activity slugs; accounts would add COPPA scope + reverse the no-backend architecture for little value). Netlify Identity is deprecated; Supabase is overkill. Optional no-backend mitigation: add export/import of progress. Owner to confirm the recommendation; revisit only if cross-device sync becomes a real, evidenced need.
