...
- [~] **2. Verify free-kit signup end-to-end.** PARTIAL 2026-06-21 — wiring verified (POST to MailerLite JSONP with `fields[email/name/language/lead_magnet]`, no-cors), the signup form renders (name+email), and all 11 lead-magnet PDFs return 200 `application/pdf` on the live site. REMAINING (owner / needs a real opt-in email + subscriber): fire one live test signup to confirm it lands in MailerLite — not done here because it creates a real subscriber. Gate A previously E2E-tested this with test emails. `EmailSignup.tsx` → MailerLite JSONP (group `storytimewitheva-signups`); confirm a real signup lands with correct `language`/`lead_magnet`/`name` and the lead-magnet PDF downloads. (See Gate A notes.)
...
- [~] **9. Test all forms: Contact, newsletter, feedback.** PARTIAL 2026-06-21 — all three render correctly (Contact: name/email/subject/message + honeypot; Feedback widget opens with rating/comment; newsletter = EmailSignup name+email), Netlify `contact`+`feedback` placeholder forms ship in `dist/index.html`, and POST wiring is code-correct (URL-encoded to `/` with matching `form-name`). REMAINING (owner / creates real tickets + notification emails): submit each live once to confirm receipt. Contact → Netlify Forms; newsletter → MailerLite; feedback → Netlify Forms (`FeedbackWidget`). Submit each live and confirm it lands + notification fires.
