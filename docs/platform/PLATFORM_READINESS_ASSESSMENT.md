...
| Release safety? | **Go with one owner action.** Every §9 pre-deploy gate runs in CI and every post-deploy gate runs automatically with results retained; `main` is branch-protected since 2026-09-18 (R-01 closed), so the gates bind in GitHub, not by practice; the remaining owner action here was enabling protection. |
...
| R-05 | the working copy lives in an iCloud-synced folder; sync created duplicate "… 2" files inside the repo twice this sprint | medium / medium | move the clone outside iCloud-synced folders or exclude it from sync; `git status` before every commit | closed 2026-09-18: clone moved to `~/Developer/storytimewitheva`, outside iCloud sync |
...
1. Owner actions: the S4-05 screen-reader pass and the copy reviews carried from Sprint 7. (R-01, R-03 and R-05 closed and the production republish drill done on 2026-09-18.)
