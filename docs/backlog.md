...
## Sprint 6 gate — RESOLVED (kept as the decision record)

All four gate items are resolved. Kept here as the record of what was decided and why,
since three of the four were content calls rather than code.
...
| B-02 | ✅ **RESOLVED** — all 20 books carry 3 `relatedActivityIds` from the approved affinity map (theme → activity category) plus age fit. Required splitting `activities.data.ts` browser-free so the ids could be validated at all. | P1 | — | Data only: no UI renders book → activity links yet. |
| B-03 | ✅ **RESOLVED — by not pairing.** All ten resources carry zero book-specific signal (no theme, age-band or title reference), so per-book curation would be fabricated. A **shared resource strip** renders the same four resources on every book page instead; `relatedResourceIds` stays empty and a test enforces that until the decision is revisited. | P1 | — | `PUNCH_LIST.md` V-04. |
