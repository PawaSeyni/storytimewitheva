...
| 4. Replace text-parsing theme filters with ID-based filters | ⬜ next |
| 5. Separate exact-age filtering from primary-fit collections in the UI | ⬜ next (live controls still read `3-5 / 6-8 / 9+`; approved doc §2 requires `8-9`) |
| 6. Canonical forward relationship fields | ⬜ blocked in part (see below) |
| 7. Build-time reverse indexes + collection counts | ⬜ |
| 8. Schema / relationship / parity / route validation | 🟨 taxonomy validation shipped; relationship validation pending step 6 |
| 9. Public collection routes for eligible themes only | ⬜ |
| 10. EN/FR/ES content + accessibility QA | ⬜ |

## Still blocked (needs a decision)
- **Resources are not modeled.** `relatedResourceIds` is approved, but there is no
  resource data model with stable IDs for it to reference. The `/resources` page is
  editorial content, not data.
- **`discussionQuestions` does not exist.** S7-005 assumes an existing model; the
  repository has none.
