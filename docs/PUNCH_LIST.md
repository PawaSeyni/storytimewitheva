...
| ID | Item | Evidence |
|----|------|----------|
| S4-01 | **S6-011: ADR-002 written, proposed, awaiting signature.** Compares local-only, optional adult accounts, and local-first sync on user value, privacy, cost, lock-in and reversibility; recommends local-only for Sprints 6–7 with an explicit evidence threshold for reopening at Sprint 8 (S8-008). Does not authorize account code. | #162 |
...
| S4-04 | **S6-018: release audit recorded** in `docs/testing/SPRINT_6_RELEASE_AUDIT.md` — 27/27 routes, 162/162 sitemap URLs, zero cookies, headers present, persistence round-trip in FR on production, no console errors. Every check-tool defect that occurred is written into the report next to the result it affected. Awaits owner signature. | #162 |
...
| S4-07 | **Pre-existing flake, logged not hidden:** `tests/e2e/analytics.spec.ts` 4.1 ("funnel events in order") fails ~1 in 3 in isolation with no changes to it or its subjects on this branch. CI's `retries: 1` has been masking it. Cause looks like a Form View / Form Start ordering race; not fixed here because it is outside Sprint 6 and deserves its own look. | open |
...
| DB-07 | **Privacy page copy is now incomplete and was deliberately NOT edited.** `Privacy.tsx` still describes local storage as "books read, want-to-read, completed activities"; it now also holds favorites, recently viewed, saved resources and preferences. Per the standing instruction, privacy-policy wording waits for the privacy specialist — flagged for that review, not changed here. | open |
