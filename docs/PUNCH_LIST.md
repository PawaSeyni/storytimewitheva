...
| PD-05 | **Unslashed URLs redirect** (`/books` 301 to `/books/`, 0.3-0.9 s on mobile; the Netlify Lighthouse plugin audits the unslashed form, so its score carries this hop while Google-referred visitors, who land on the slashed canonical, do not). Writing `<route>.html` twins was tried in #208 and reverted: with both files present Netlify redirects the canonical `/books/` to `/books` instead, which would flip every indexed URL. Owner decision: either disable Netlify's Pretty URLs post-processing (both forms then serve 200 and the canonical tag disambiguates) or move the canonical to the unslashed form site-wide (sitemap, canonical, hreflang, then twins). Not a code change. | open |
