...
Last updated 2026-09-18 against `main` @ `76eb2aa`.
...
| DA-02 | **One canonical URL form: the trailing slash.** `withTrailingSlash` in `src/lib/locales.ts` is applied by `LocalizedLink`, the language switch and the guide redirect, and `journey.ts` targets carry it, so every internal link, the games' nav and continue links, canonical, hreflang and the sitemap agree on `/books/`. Netlify Pretty URLs is back ON so `/books` answers a single 301 to `/books/` (this supersedes PD-05, which had turned it off to avoid the hop: the auditor's point is that a client-side visit to `/books` and a direct load of `/books/` were two pages in Plausible). Files, `/download/…` function endpoints and `/games/*.html` are never slashed. Acceptance: only the slashed URL returns 200, the other a single 301; unit test for the helper; no unslashed page link left in the prerendered site. | PR |
| DA-03 | **Indexable resource layer.** The `/free/*` campaign pages stay noindex (thin, form-first, Pinterest). The six parent guides at `/resources/<slug>/` were already indexable with canonical, hreflang, Article and BreadcrumbList schema, GuideLinks to books and the signup form inside, but ran 330-660 words; every guide is now 800+ words in EN, ES and FR (new sections in the same voice), and two guides the audit asked for were written from scratch: **A Bedtime Reading Routine That Actually Sticks** (pairs with the Reading Routine Chart) and **How to Use Bilingual Flashcards With Young Children** (pairs with the flashcards set). All eight are linked from the footer (new Parent guides column), the homepage (three featured guides), the Resources index and the related books. `tests/funnel/guides-length.test.mjs` locks the 800-word floor per language. The bodies moved out of JavaScript into per-language JSON assets (`src/data/guides/<lang>.json`, emitted hashed by Vite, fetched once and read with `use()`, preloaded with the route chunk): 20,000 words of prose had pushed the total-JS budget past its fail line, and text is content, not code. The Article chunk went from 49 KB to 3 KB gzip. | PR |
| DA-04 | **Primary funnel in Plausible**: pageview goal `Guide View` (pattern `/**resources/*/`, all three languages, guide pages only) and the funnel **Guide → lead → book → Amazon** (Guide View → Lead Created → Book View → Purchase Click). Details in `docs/plausible-setup.md`. Remaining owner action from the audit: connect Google Search Console. | dashboard |
| DA-05 | **Search Console was already connected**: URL-prefix property for `https://storytimewitheva.com/`, verified since June by the meta tag, sitemap submitted and read the same day as the guides deploy. Read on 2026-09-18: 99 indexed, 52 not indexed (25 discovered-not-crawled, 13 crawled-not-indexed, 8 redirects, 3 alternates, 2 robots-blocked by design, 1 stray `/Home` 404), 7 clicks and 206 impressions since July, the only real query `eva gallo`. Full reading and the monthly watch list in `docs/seo/SEARCH_CONSOLE_2026-09-18.md`. | recorded |
| DA-06 | **`/Home` 404 fixed at the server** (Search Console's one Not-found URL, a stray inbound link with a capital H): Netlify answers `/home` and `/Home` with a single 301 to `/`; the smoke suite asserts it on every deploy. The SPA's client-side redirect used to run behind a 404 status, which is what Google recorded. | #219 |
| DA-07 | **Stale `Amazon Click` goal deleted from Plausible.** No code fired it (the Amazon-click conversion is `Purchase Click` with destination `amazon`); historical event data is untouched and the goal can be re-added under the same name if ever needed. Recorded in `docs/plausible-setup.md`. | dashboard |
| SR-01 | **Seasonal deploys automated (R-06).** `seasonal-rebuild.yml` runs at 00:30 UTC on each window boundary from `src/data/collections.ts` (opens: 06-15, 08-15, 11-01; day after closing: 09-01, 10-01, 01-01), posts to the Netlify build hook `seasonal-rebuild`, waits for the build, verifies the live sitemap lists exactly the collections open that day, then runs the production smoke suite. `tests/funnel/seasonal-rebuild.test.mjs` fails if a window changes without its cron line. Drilled by hand on 2026-09-18: hook build triggered, Back to School read as open and the other two seasons closed, sitemap matched with 18 indexable collections, 46 smoke tests green. The drill exposed that a coinciding push build could satisfy the wait, so the wait now requires a build newer than the hook request (#221). Runbook step 6 covers a missed boundary. Next boundary: 2026-10-01, when Back to School closes. | #220 #221 |
...
| AC-04 | **Owner decision (2026-09-16): the French Mira's Thousand Cubes links the Kindle ASIN `B0HFV1LKM4`**, not the paperback ISBN, after the duplicate French Kindle `B0HFB3R85B` (the one linked to paperback 1996972855 in KDP) was unpublished. Exception to AC-02, on purpose: the surviving Kindle is the edition the owner wants the Buy link to land on. | PR |
...
| ID | Item | Evidence |
|----|------|----------|
| AS-01 | **The table is the contract.** Each activity's "best book connection" lists the activity FIRST in that book's `relatedActivityIds`, so the book page shows it first and the activity page continues to it. Kindness Ripple pairs with both The Butterfly Effect and The Sparrow Who Saved the Forest; The Patient Maker Passport with Heidi's Journey to Mastery, Mira's Thousand Cubes and The Sanding Block. Locked by `tests/funnel/activity-studio.test.mjs` (only those books reference each activity) and `tests/e2e/studio.spec.ts` (rendered pairing links per page). | PR |
| AS-02 | **Characters match the books**: Hawel, Pixel and the Color Keeper of Harmonia (Mix-It Lab); Cirro and Pawa the Sun (Feelings Weather); Diego on the cliff with his family on the beach (Brave Plan); Luna and Grandpa Chen, cumulus/cirrus/stratus/nimbus (Cloud Detective); Maya's shadow, with a Maya cut-out in the paper kit (Shadow Theatre); the butterfly and the sparrow as two of the kind acts (Kindness Ripple); Sofia, her grandmother's fig tree and Mr. Costas (Story Quilt); Heidi's broom, Mira's thousand cubes and the sanding block (Maker Passport). | PR |
| AS-03 | **Ages and priorities from the table**: 4-7, 3-7, 4-8, 4-8, 3-7, 4-9, 5-9, 5-9. Every pairing passes the existing age-overlap relationship test. | PR |
| AS-04 | **Trilingual features kept**: the color recipe card and the cloud journal print the words in all three languages; everything else follows the page language. Read-aloud in Shadow Theatre uses the browser's own speech synthesis in the page locale (`intlLocale`), no network. | PR |
| AS-05 | **Two activities keep state on the device**: Kindness Ripple (a seven-day chart) and Maker Passport (practice across sessions), through the storage adapter (`kindnessRipples`, `makerPassport` LEGACY keys), never a cookie; both are in the cookie-free and storage-denied suite. The other six are stateless by design. | PR |
| AS-06 | **Print**: the instructions panel and buttons carry `print:hidden`; only the take-home card prints, alongside the site's existing chrome rules. | PR |
| AS-07 | **Search ranking**: "Kindness" (the collection) now outranks "Kindness Ripple Challenge" (the activity): a title that IS the query scores above one that starts with it. | PR |
| AS-08 | **Budget**: eight lazy chunks add ~11 KB gzip to the total JavaScript (entry unchanged at 104 KB); the total-JS warning threshold moved from 320 to 340 KB (fail stays 350, owner-owned). | PR |
| AS-09 | **Not done**: the studio's hero and sticker artwork (no page uses them; 9.7 MB of PNG), a studio landing page (the Activities index is the hub), and a home-page preview slot for the new activities (owner choice). | open |
...
| ID | Item | Evidence |
|----|------|----------|
| RC-01 | **23 React Compiler readiness findings, all fixed; the six compiler rules now run at the plugin's recommended `error` level.** `static-components`: `StepHeader` moved to module scope in CharacterWorkshopDemo (it was remounting and resetting its subtree on every render). `immutability` + `use-memo`: the toast function object is built once by a module-scope factory inside `useMemo`; `useMemo(() => buildIndex(), [])` in TapToTranslate. `refs`: ReadAlong syncs `tokensRef` in an effect and derives the highlight from `speaking` instead of resetting it in an effect. `set-state-in-effect`: storage and URL reads moved into `useState` initializers (EmailSignup `?signup=`, Pixel preference, Adventure Journal entries, Coloring gallery); Navbar's mobile menu is open only on the path it was opened on, so navigation closes it without an effect; Bingo, Puzzles and Word Explorer reset on language change with React's adjust-state-during-render pattern (Bingo's board rebuild too). `purity`: the random starting puzzle is chosen in the initializer. Verified: lint 0 hook findings, `tsc -b`, 216 unit + 237 SEO tests, 86 e2e, and all 84 visual screenshots pixel-identical to the pre-change baselines (the one diff caught during the work, the profile mascot hidden under the prerender, was reverted before merge). | PR |
...
| GA-02 | **Sprint 5 audit approved** by the owner 2026-09-10. GM-05 (Plausible key + properties) still gates the numeric baseline and the experiments. | #197 |
...
| GM-05 | **Owner action, blocks the numeric baseline and both experiments**: create a Plausible Stats API key (set `PLAUSIBLE_API_KEY` where the report runs) and enable the custom properties listed in the dictionary in the Plausible dashboard. Then run the 28-day baseline and approve it. | resolved 2026-09-18: checked first, both already existed: a Stats API key `storytimewitheva funnel report (local)` (in the git-ignored `.env`, used for `docs/analytics/REPORT_2026-09-18.md`) and every dictionary property enabled under Custom properties in the dashboard; the 28-day rerun is scheduled for 2026-10-16. Baseline approved by the owner on 2026-09-18 (`docs/analytics/BASELINE_2026-09.md`); the two experiments may be activated when the owner sets `startAt` |
...
| PO-08 | **Clone moved out of iCloud sync (R-05), 2026-09-18**: `~/Desktop/storytimewitheva` → `~/Developer/storytimewitheva` (Desktop is under iCloud's CLOUDDESKTOP sync, which had been creating `name 2.ext` duplicates inside the working tree and once inside `.git`). Working tree was clean before the move; git, remote and `node_modules` came along intact; the scheduled funnel-report task, RUNBOOK, the site skill note and `.claude/launch.json` now point at the new path; a pointer note was left on the Desktop. The `* 2*` cleanup step before commits is no longer needed. | local |
| PO-06 | **Owner action: scope `MAILERLITE_GROUP` for the `deploy-preview` context** to a test group so preview signups stop creating real subscribers. Security review R-03. | resolved 2026-09-18: checked first, no test group existed; MailerLite group `storytimewitheva-preview-test` (id 198977153473185382) created and `MAILERLITE_GROUP` set to it for the `deploy-preview` and `branch-deploy` contexts only; production keeps the default `storytimewitheva-signups`. Proven on a deploy preview: a test signup appeared in the test group and not in the real one |
...
| PD-04 | **`/books` Lighthouse mobile 71, closed: the score was a fallback flash, not the covers.** Measured (Lighthouse mobile, production, three runs): 67-72, CLS 0.58-0.67, LCP 2.7-3.3 s; the layout-shift trace showed one event, the footer pulled from 17,000 px down into the viewport and back. Cause: React.lazy commits the Suspense fallback even with the route chunk in cache, so every prerendered non-home page was swapped for the fallback and re-rendered; the covers were never the problem (re-encoding them at q80 saves 8%, not taken). Fix: `main.tsx` preloads the current route's chunk before the first render (`src/routeLoaders.ts`, 3 s cap) and the route components read it with React 19's `use()` (`src/lib/chunk.tsx`), which replays synchronously on a fulfilled promise where `lazy()` never does. Deploy preview, same Lighthouse runs: 93-99, CLS 0, LCP 1.6-2.7 s. Production after the deploy (fbc1f0a, three runs): 90-99, CLS 0 on every route probed, 44 smoke tests green. Gates: e2e `no-fallback-flash.spec.ts` (CLS < 0.01 and no fallback commit on six routes with the CPU throttled ×4), unit tests for the preload helper, 84 visual screenshots identical. | #208 |
| PD-05 | **Superseded by DA-02 (2026-09-18): Pretty URLs back on, internal links slashed.** Was: Unslashed URLs no longer redirect: Netlify Pretty URLs turned off (owner decision, 2026-09-10). `/books` was a 301 to `/books/` (0.3-0.9 s on mobile; the Netlify Lighthouse plugin audits the unslashed form). `<route>.html` twins were tried in #208 and reverted: with both files Netlify redirected the canonical `/books/` to `/books` instead. With Pretty URLs off (site setting, recorded in the disaster-recovery runbook step 4) both forms serve the same bytes with 200 and the canonical tag keeps `/books/`; verified on production for unslashed and slashed EN/FR/ES pages, book, activity, profile, landing, games and 404. Lighthouse mobile on `/books` unslashed: 88-100, redirects none (was 70-72 with a 935 ms hop). 44 smoke tests green. | site setting |
...
| N-04 | **S3-018/019/020 have no baseline artifacts.** No Search Console baseline, route/metadata inventory or post-deploy crawl record exists, and Sprint 3's definition of done requires them. | resolved 2026-09-18: `docs/seo/SEARCH_CONSOLE_2026-09-18.md` is the Search Console baseline (99 indexed, 52 not, 7 clicks and 206 impressions since July, the only real query `eva gallo`), with a monthly watch list | Owner/data task more than a code task. |
