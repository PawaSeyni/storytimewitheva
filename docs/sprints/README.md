...
| [5](SPRINT_5_TECHNICAL_IMPLEMENTATION.md) | Monetization, growth, product intelligence | S5-001 … S5-024 — complete; numeric baseline and experiment activation wait on Plausible data access (GM-05) |
...
| S4-010 · S4-011 | ✅ shipped | `src/lib/journey.ts` + `ContinueJourney` (#166); games continuation (#167). Row was stale |
| S5-001 | 🟡 instrumentation baseline | `docs/analytics/BASELINE_2026-09.md`: funnels defined, every surface instrumented, gaps named; numeric baseline blocked on Plausible API access and custom-property enablement (owner) (#191) |
| S5-002 · S5-003 | ✅ shipped | Buy group labelled and separate from read/listen/save controls; `Purchase CTA View` once when viewable; `Purchase Click` carries book, destination, placement (detail/card) and the edition language behind the link (`editionLang` on the localized book) (#191) |
| S5-005 · S5-006 | ✅ shipped | `EmailSignup` takes a placement (home, landing, books, activities, resources, about) that travels on every funnel event; localized contextual line per placement (#191) |
| S5-007 | ✅ shipped | Signup success continues into three featured books or the catalog, tracked as `Continue Journey` {signup-success}; funnel `free-bundle-loop` (#191) |
| S5-012 · S5-013 · S5-014 · S5-017 · S5-004 | ✅ tooling | `npm run report:funnels` (Plausible Stats API v2 or a fixture): every funnel with step rates, segments by placement/locale/age band/theme/landing page, minimum-sample warnings, book classification only above the sample, unknown ids surfaced; tested on a fixture (#191) |
| S5-020 | ✅ shipped | `ShareButton` on book pages: native share sheet or copy-link fallback with a polite announcement; `Share` carries the target kind only (#191) |
| S5-021 · S5-022 | ✅ shipped | `src/analytics/events.ts` versioned dictionary (alias, kind, required/optional, allowed values, owner, privacy, reserved) generating `docs/analytics/EVENT_DICTIONARY.md`; tests: runtime taxonomy = dictionary, every call site passes every required key, `validateEvent`, funnel steps reference defined events, doc current; e2e: impression once and before click, placement on every signup event, loop continuation, share (#191) |
| S5-024 | ✅ audited | `docs/testing/SPRINT_5_RELEASE_AUDIT.md`: 60 book pages on production, 294/294 Amazon links tagged and safe, 60/60 disclosures and edition mappings, function validating, smoke 44 and `test:prod-audit` 75 passed (now incl. commerce + experiments specs). Plausible receipt not verifiable from the repo (GM-05). **Release accepted by the owner 2026-09-10** (#197) |
| S5-015 · S5-016 | ✅ shipped | Loop instrumented end to end: book → activity (`Continue Journey` from the related-activities strip, reason `related`), activity → book (existing continuation), related-book clicks carry their ranking tier (`Recommendation Click`, placement `related`). More Like This keeps the approved editorial + theme policy; `tests/seo/monetization.test.mjs` asserts localized, resolvable, non-duplicate, non-self links (#193) |
| S5-018 | ✅ documented | `docs/analytics/MERCHANDISING.md`: the approved controls (`featured`, catalog order, `bookOrder`, `relatedBookIds`, seasonal window) and what is not approved; tests lock featured count, published state and order (#193) |
| S5-019 | ✅ standard | `docs/analytics/NEWSLETTER_ATTRIBUTION.md`: link standard for MailerLite emails (stable `/download/`, localized routes, three UTMs); the site attaches UTMs to every event; owner applies it in the dashboard (#193) |
| S5-023 | ✅ shipped | Retailer links (affiliate tag, new tab, noopener) on every book page in three locales, disclosure beside the Buy group in the page language, edition mapping per locale, CTA states, experiment fallback: all asserted on the prerendered dist (#193) |
| S5-008 · S5-011 | ✅ shipped | `src/analytics/experiments.ts` registry with governance fields; `src/analytics/experimentEngine.ts` (validator, FNV-1a deterministic assignment, eligibility by locale/route/dates, kill switch by status, bots → control) + `src/lib/experiments.ts` React binding (unit id through the storage adapter, memory when denied; exposure once per assignment when viewable; conversion props only with an exposure). CI rejects an incomplete non-draft experiment. `docs/experiments/README.md` (#192) |
| S5-009 · S5-010 | 🟡 draft, wired | EXP-001 book CTA hierarchy (`listen-first` variant renders a read-aloud primary action above the Buy group) and EXP-002 contextual newsletter CTA (`plain` hides the contextual line) are wired into the surfaces and documented with hypothesis, owner, audience, duration, sample, guardrails, analysis and stop rules. Both stay `draft` (control for everyone, no exposure) until the numeric baseline is approved (gate 11.1) (#192) |
